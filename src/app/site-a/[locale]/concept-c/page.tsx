...
export default function ConceptCPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white/20 overflow-x-hidden relative">
      ...
      {/* (Framer Motion animations are disabled on mobile to maximize render frame-rates) */}
      <motion.div 
        animate={isMobile ? undefined : { opacity: [0.25, 0.45, 0.25], scale: [1, 1.05, 1] }}
      ...
      />
      <motion.div 
        animate={isMobile ? undefined : { opacity: [0.2, 0.35, 0.2], scale: [1, 1.1, 1] }}
        transition={isMobile ? undefined : { duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }}
        style={{ opacity: 0.25 }}
        className="fixed top-[-5%] right-[-10%] w-[32vw] h-[32vw] bg-emerald-500/15 rounded-full blur-[120px] pointer-events-none z-0"
      />
      <motion.div 
        animate={isMobile ? undefined : { opacity: [0.25, 0.4, 0.25], scale: [1, 1.05, 1] }}
        transition={isMobile ? undefined : { duration: 12, repeat: Infinity, ease: "easeInOut", delay: 4 }}
        style={{ opacity: 0.3 }}
        className="fixed top-[45%] left-[-15%] w-[42vw] h-[42vw] bg-violet-600/20 rounded-full blur-[140px] pointer-events-none z-0"
      />
      <motion.div 
        animate={isMobile ? undefined : { opacity: [0.15, 0.28, 0.15], scale: [1, 1.15, 1] }}
        transition={isMobile ? undefined : { duration: 14, repeat: Infinity, ease: "easeInOut", delay: 6 }}
        style={{ opacity: 0.2 }}
        className="fixed top-[35%] right-[-15%] w-[35vw] h-[35vw] bg-amber-500/12 rounded-full blur-[130px] pointer-events-none z-0"
      />
      <motion.div 
        animate={isMobile ? undefined : { opacity: [0.3, 0.48, 0.3], scale: [1, 1.08, 1] }}
      ...
      />
      <motion.div 
        animate={isMobile ? undefined : { opacity: [0.2, 0.38, 0.2], scale: [1, 1.12, 1] }}
        transition={isMobile ? undefined : { duration: 11, repeat: Infinity, ease: "easeInOut", delay: 3 }}
        style={{ opacity: 0.25 }}
